# ♟DB서버 이중화 Active, Stanby 구축하기

## 🎃 개요
> VM1에서 스프링 서버와 함께 운영중인 Mysql DB를 1시간마다 덤프를 추출합니다. 해당 덤프를 VM2의 DB서버로 마이그레이션하는 작업을 자동화 합니다. 운용중인 DB를 매 시간 백업하여 재해상황을 대비합니다.
...
## 🎨고찰
> DB를 이중화하고 Active, Stanby로 운영하면서 데이터베이스의 싱크를 맞추기 위해 크론탭으로 배치 마이그레이션을 진행하는 작업을 하였습니다. 사용자가 적거나 사내에서 사용하는 서비스일 경우 데이터의 트랜잭션이 많지 않아서 시간마다 배치를 하는 것이 적합하겠지만 규모가 크고, 대규모 서비스라면 실시간으로 DB를 마이그레이션할 수 있는 방법이 필요할 것이라고 생각합니다.